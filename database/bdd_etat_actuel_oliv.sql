...
CREATE TABLE IF NOT EXISTS `chomage` (
  ...
  `ch_number` varchar(50) DEFAULT NULL, -- est un taux de chomage
...
-- demandeurs d'emploi par commune
...
CREATE TABLE IF NOT EXISTS `logements` (
  ...
  KEY `log_type_fk_idx` (`log_type_id`)
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 AUTO_INCREMENT=1 ;
...
ALTER TABLE `logements`
  ...
  ADD CONSTRAINT `log_type_fk` FOREIGN KEY (`log_type_id`) REFERENCES `log_type` (`log_type_id`) ON DELETE NO ACTION ON UPDATE NO ACTION;
...
tr_number int,
zone_no int,
FOREIGN KEY (zone_no) REFERENCES  zone_demploi(zone_no),
cat_id int,	
FOREIGN KEY (cat_id) REFERENCES categorie_age(cat_id)
);
...
dipl_year int,
dipl_id int,
FOREIGN KEY (dipl_id) REFERENCES diplome(dipl_id),
cat_id int,
FOREIGN KEY (cat_id) REFERENCES categorie_age(cat_id),
pt_id int,
FOREIGN KEY (pt_id) REFERENCES pop_type(pt_id)
);